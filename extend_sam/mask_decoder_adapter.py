...
from .mask_decoder_heads import SemSegHead,SemSegHeadMulti
...
class SemMaskDecoderAdapter(BaseMaskDecoderAdapter):
    def __init__(self, ori_sam: Sam, fix=False, class_num=20):
        ...
        self.decoder_head = SemSegHeadMulti(transformer_dim=self.sam_mask_decoder.transformer_dim,
                                       num_multimask_outputs=self.sam_mask_decoder.num_multimask_outputs,
    ...
    def forward(self, image_embeddings,multi_image_embeddings, prompt_adapter, sparse_embeddings, dense_embeddings, multimask_output=True,
                attn_sim=None,scale=0):
        src, iou_token_out, mask_tokens_out, src_shape = self.decoder_neck(image_embeddings=image_embeddings,
                                                                           image_pe=prompt_adapter.sam_prompt_encoder.get_dense_pe(),
                                                                           sparse_prompt_embeddings=sparse_embeddings,
                                                                           dense_prompt_embeddings=dense_embeddings,
                                                                           multimask_output=multimask_output,attn_sim=attn_sim )
        masks, iou_pred = self.decoder_head(src,multi_image_embeddings,dense_embeddings, iou_token_out, mask_tokens_out, src_shape, mask_scale=scale)
