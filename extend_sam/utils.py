...
class mIoUOnline:
    ...
    def addBefore(self, pred_mask, gt_mask):
        obj_mask = gt_mask < 255
        correct_mask = (pred_mask == gt_mask) * obj_mask
        for i in range(self.class_num):
            IoUsingle = np.sum((gt_mask == i) * correct_mask) / (np.sum((pred_mask == i) * obj_mask) + np.sum((gt_mask == i) * obj_mask) - np.sum((gt_mask == i) * correct_mask) + 1e-10) * 100
            if IoUsingle>10:
    ...
    def add1(self, pred_mask, gt_mask):
        obj_mask = gt_mask < 255
        correct_mask = (pred_mask == gt_mask) * obj_mask
        for i in range(self.class_num):
            IoUsingle = np.sum((gt_mask == i) * correct_mask) / (np.sum((pred_mask == i) * obj_mask) + np.sum((gt_mask == i) * obj_mask) - np.sum((gt_mask == i) * correct_mask) + 1e-10) * 100
            if i>0 and IoUsingle>0:
                print("class"+str(i)+"++++"+str(IoUsingle))
            self.P[i] += np.sum((pred_mask == i) * obj_mask)
            self.T[i] += np.sum((gt_mask == i) * obj_mask)
            self.TP[i] += np.sum((gt_mask == i) * correct_mask)
